...
def calc_sWeights(Functions,pars,data,testdata,plot=False,bins=100,write=False,save_err=False):
    ...
    Functions : list of callable functions
        The "PDFs" defining the different species in data. 
        Assumes the first parameter (p[0]) to be the scaling for the function to fit data (number of expected events). Needs to be of the form: f(x,p). 
    ...
    testdata : N length vector
        Vector containing the data which needs to be seperated into different species.
        N is the number of data points.

    plot : False, 'True' or 'Combined', optional
        If plot='True' plots the **testdata** a histogram and shows the individual contributions
    ...
    # Creating a PDF for each species in data (to be called later)
    pdfFuncs = []
    expectedNEvents = []
    for i_s in range(nSpecies):
    ...
    #  e.g. the value of each species PDF for each event.
    ...
    pdfValue = np.zeros([nSpecies,nEvents],dtype=float)
    denom = np.zeros([nEvents],dtype=float)
    iclist = np.zeros([nSpecies,nSpecies],dtype=float)
    ...
    for i_e in range(0,nEvents):
        ...
        # Loop over species (e.g. PDFs)
        for i_s in range(0,nSpecies):

            # Calculate PDF value for this species for this event
            pdfValue[i_s,i_e] = pdfFuncs[i_s](dataN) / expectedNEvents[i_s] 

            # Calculate the contribution to the normalisation denominator term for this event for this species and add to running sum
    ...
    # Calculating sWeights for each event, for each species (using eqn 14 in [1])

    # Create the array to fill
    sWeight = np.zeros([nEvents,nSpecies],dtype=float)

    # Loop over events
    for i_e in range(0,nEvents): #e in eqn 14 in [1]

        # Loop over species (n in eqn 14 in [1])
        for i_s_n in range(0,nSpecies):

            # Calculate this sWeight (there is a loop over species, j from eqn 14 in [1], happening here)
            sWeight[i_e,i_s_n] = np.sum( [ covMatrix[i_s_n,i_s_j]*pdfValue[i_s_j,i_e] for i_s_j in range(0,nSpecies) ] ) / denom[i_e]
    ...
    err = np.zeros([len(steps),nSpecies])
    for i_s in range(0,nSpecies):
    ...
    if plot == 'True' or plot == 'Combined':
        ...
        color = cm.rainbow(np.linspace(0,1,nSpecies))
    ...
    # Plotting the entire dataset and the individual contributions from the different species
